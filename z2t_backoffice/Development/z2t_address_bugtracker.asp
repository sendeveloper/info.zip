...
<body>
  <!--#include virtual="/z2t_Backoffice/includes/BodyParts/header.inc"-->
                ...
                <div style="margin: auto; width: 100%; font-size: 1em; font-style: normal; line-height: normal; font-weight: bold; font-variant: normal; text-transform: none;">
                  <select id="selSort" name="selSort" style="width: 10em; float: right; margin-right: 9em" onchange="window.location='<%request.servervariables("URL")%>?BugStatus=' + this.value">
                    <% 
                      Dim rs, rsDDL, strSQL, conn

                      set rsDDL = server.createObject("ADODB.Recordset")
                      set conn = server.CreateObject("ADODB.Connection")  

                      strSQL = "SELECT * FROM dbo.z2t_Types_repl WHERE Class = 'BugStatus'"
                  ...
                  <% 
	                ...
	                ' SQL Query
	                strSQL = "SELECT * FROM dbo.z2t_bugtracking_addresses WHERE DeletedDate IS NULL"

                    If Not Request("BugStatus") = "" And Not Request("BugStatus") = "All" Then
                      strSQL = strSQL & " AND Status ='" & Request("BugStatus") & "'"
                  ...
                  <tr style="text-align: left; width: 100%; vertical-align: middle; margin-left: .5em; background-color: white">
                    <td style="text-align: left; width: 538; height: 2em; vertical-align: middle; background-color: white; border: 1px solid black"><span style="margin-left: 1em"><%=rs("StreetAddress") & "&nbsp;" %><%=rs("City")%>,<%=rs("State")%>&nbsp;<%=rs("PostalCode")%></span></td>
                    ...
                    <td style="text-align: center; width: 100; height: 2em; vertical-align: middle; background-color: white; border: 1px solid black"><%response.write(rs("Status") & "&nbsp;")%></td>
                    <td style="text-align: center; width: 104; height: 2em; vertical-align: middle; background-color: white; border: 1px solid black">
                      <span class="noPrint"><a style="font-size: 1em; text-decoration: none; font-weight: bold" href="z2t_add_address_bug.asp?id=<%=rs("ID")%>">Edit</a></span>
                    </td>
                  </tr>
                  <%
              ...
              <div class="noPrint" style="margin: 0 auto; width: 20%">
              	<a class="button" href="javascript:void(0)" onclick="window.print()">Print</a>
              </div>
