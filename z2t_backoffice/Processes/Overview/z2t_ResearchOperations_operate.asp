...
<%
  Case "add-jur"
    ...
    Case "2"
      For Each taxtype in Split(taxtypes, ",")
        sqlText = "insert into z2t_UpdateRates.dbo.z2t_TaxData(State, County, TaxType, Rate, JurRate, JurType, JurSize, EffFrom, EffTo, CreatedBy, CreatedDate) select state = '" & Request("state") & "', county = '" & Request("county") & "', taxtype = " & taxtype & ", rate = " & Request("rate") & ", jurrate = " & Request("jurrate") & ", jurtype = " & Request("jurtype") & ", jursize = " & Request("jursize") & ", efffrom = '" & Request("efffrom") & "', effto = '" & Request("effto") & "', createdby = '" & EmpLogin &  "', createddate = getdate()"
        'Response.Write(sqlText & vbcrlf & vbcrlf)
        Call Sql(sqlText)
      Next
