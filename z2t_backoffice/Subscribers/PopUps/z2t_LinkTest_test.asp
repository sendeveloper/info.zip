...
<%
  ...
  AllAjaxURL = PathURL & "z2t_DatabaseTest.asp?zip=" & RequestZip & "&usr=" & Request("Usr") & "&pwd=" & Request("Pwd")
%>
  ...
  function pressEnter() 
    ...
    function runTest()
    {
        ...
        var timeoutFn = function(ignore, ignore2, timer) 
        {
            clearTimeout(timer);
            for (var i=1;i<15;i++)
                linkToError("ajax timeout", "Timed out with no response.")(document.querySelectorAll("#city" + i)[0].firstChild);
        }

        var http = xhr(ajaxURL, successFn, errorFn, 10000, timeoutFn);
    ...
    function getUpdateResponse(ignore, rrr)
    {
        var rr = rrr.querySelectorAll("zip_code_lookup");
        for (var i=0;i<14;i++)
        {
            var ServerID = serverSequence[i];
            var r = rr[i];
            if (!r) {
    ...
    function errorfn(r, ignore) 
    {
        for (var i=1;i<15;i++)
        {
            var element = document.querySelectorAll("#city" + i)[0];
            if (r && r.length > 0) {
    ...
    {
        var URL = 'z2t_LinkTest_test.asp?zip=' + document.frm.zip.value +'&usr=<%=Request("usr")%>' +'&pwd=<%=Request("pwd")%>&v=1.1';
        window.location = URL;
    }
