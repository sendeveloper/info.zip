...
<%
''	response.Write(Request("zip") &vbcrlf  & request("usr") &vbcrlf & request("pwd"))
  If isnull(Request("zip")) or Request("zip") = "" Then
    RequestZip = 82009
  Else
    RequestZip = Request("zip")
	''response.Write(requestzip &vbcrlf  & request("usr") &vbcrlf & request("pwd"))
	''response.End()
  End If
    
  AjaxLoadingGif = "'http://info.zip2tax.com/z2t_backoffice/images/AjaxLoading.gif' alt='Loading'"

  PathURL = "http://info.zip2tax.com/z2t_Backoffice/subscribers/popups/Ajax/"

  Dim AjaxURL(20), ActionURL(20)
  AjaxURL(1) = PathURL & "z2t_DatabaseTest_Barley2_link.asp?zip=" & RequestZip & "&usr=" & Request("Usr") & "&pwd=" & Request("Pwd")
  'AjaxURL(2) = "https://casper09.zip2tax.com" 'PathURL & "z2t_DatabaseTest_Philly01.asp?zip=" & RequestZip & "&usr=" & Request("Usr") & "&pwd=" & Request("Pwd")
  AjaxURL(2) = PathURL & "z2t_DatabaseTest_Philly01.asp?zip=" & RequestZip & "&usr=" & Request("Usr") & "&pwd=" & Request("Pwd")
  AjaxURL(2) = ""
  AjaxURL(3) = "http://www.zip2tax.com/Link/Lookup_XML.asp?zip=" & RequestZip & "&usr=" & Request("Usr") & "&pwd=" & Request("Pwd")
  AjaxURL(4) = PathURL & "z2t_DatabaseTest_Barley2_link_mysql.asp?zip=" & RequestZip & "&usr=" & Request("Usr") & "&pwd=" & Request("Pwd")
  AjaxURL(5) = PathURL & "z2t_DatabaseTest_Philly01_mysql.asp?zip=" & RequestZip & "&usr=" & Request("Usr") & "&pwd=" & Request("Pwd")
  
  AjaxURL(6) = PathURL & "z2t_DatabaseTest_Philly02_api.asp?zip=" & RequestZip & "&usr=" & Request("Usr") & "&pwd=" & Request("Pwd")
  AjaxURL(7) = PathURL & "z2t_DatabaseTest_Philly04_api.asp?zip=" & RequestZip & "&usr=" & Request("Usr") & "&pwd=" & Request("Pwd")
  AjaxURL(8) = PathURL & "z2t_DatabaseTest_Casper06_api.asp?zip=" & RequestZip & "&usr=" & Request("Usr") & "&pwd=" & Request("Pwd")
  ...
  AjaxURL(11) = PathURL & "z2t_DatabaseTest_Philly05_api.asp?zip=" & RequestZip & "&usr=" & Request("Usr") & "&pwd=" & Request("Pwd")
...
<head>
  ...
  <script type="text/javascript">
    ...
    var ajaxURL = ['', 
						...
						'<%=AjaxURL(11)%>', '<%=AjaxURL(12)%>', '<%=AjaxURL(13)%>', '<%=AjaxURL(14)%>', '<%=AjaxURL(15)%>'];
    var serverSequence = [1, 13, 12, 9, 10, 3, 4, 5, 6, 7, 11, 8];
    var nextServer = 0
...
//	 alert(ajaxURL[ServerID]);
    ...
    function getUpdateResponse(ignore, r, ServerID)
        {
        if (!r) {return errorfn(ignore, r, ServerID);}
        if (r.querySelectorAll("error_code")[0] && r.querySelectorAll("error_code")[0].nodeValue && (["", "0"].indexOf(r.querySelectorAll("error_code")[0].nodeValue) = -1)) {return errorfn(ignore, r, ServerID)}
        if (r.querySelectorAll("error_message")[0] && r.querySelectorAll("error_message")[0].firstChild.nodeValue && (["", "No Errors"].indexOf(r.querySelectorAll("error_message")[0].firstChild.nodeValue) < 0)) {return errorfn(ignore, r, ServerID)}
  ...
  <tr>
    ...
    <table width="100%" border="0" cellspacing="2" cellpadding="2">
      ...
      <tr valign="top">
	    ...
	    <%=DisplayTestBlock (1, "Barley2", "Link", 3)%>
	    <%=DisplayTestBlock (13, "Philly01", "LinkAPI", 9)%>
	    <%=DisplayTestBlock (3, "Philly01", "Req Var", 5)%>
	    <%=DisplayTestBlock (12, "Philly03", "Website", 3)%>
	    <%=DisplayTestBlock (9, "Philly05", "Website (Staging)", 3)%>
	    <%=DisplayTestBlock (10, "Casper06", "Website", 3)%>		
      </tr>
    </table>
    ...
    <table width="100%" border="0" cellspacing="2" cellpadding="2">
      ...
      <tr valign="top">
	    <%=DisplayTestBlockHead%>
	    <%=DisplayTestBlock (4, "Barley2", "Link", 6)%>
    ...
    <table width="100%" border="0" cellspacing="2" cellpadding="2">
      ...
      <tr valign="top">
        ...
        <td width="14%">&nbsp;</td>
		<td width="14%">&nbsp;</td>
	    <%=DisplayTestBlock (11, "Philly05", "API (Staging)", 3)%>
...
<%
	Function DisplayTestBlockHead
        ...
        s = s & "    <tr><td align='right'>Post Office City:</td></tr>" & vbCrLf
        s = s & "    <tr><td align='right'>County:</td></tr>" & vbCrLf
        s = s & "    <tr><td align='right'>State:</td></tr>" & vbCrLf
	...
	Function DisplayTestBlock (TestID, ServerName, Purpose, HelpID)
        ...
        s = s & "    <tr><td align='center' id='city" & TestID & "'><img class='loading' src=" & AjaxLoadingGif & " /></td></tr>" & vbCrLf
        s = s & "    <tr><td align='center' id='county" & TestID & "'>&nbsp;</td></tr>" & vbCrLf
        s = s & "    <tr><td align='center' id='state" & TestID & "'>&nbsp;</td></tr>" & vbCrLf
        s = s & "    <tr><td align='center' id='shipping" & TestID & "'>&nbsp;</td></tr>" & vbCrLf
		

		If TestID = 9 or TestID = 11 Then
